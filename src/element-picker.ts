...
export function createElementPicker(onPick: (element: HTMLElement) => void): ElementPicker {
	...
	function ensureHighlight(): HTMLElement {
		if (!highlight) {
			...
			// pointer-events must be off, or the z-index 2e9 box sits under the cursor and
			// `elementFromPoint` returns the highlight instead of the page element beneath it.
	...
	function position(element: HTMLElement): void {
		...
		// Mirrors Astro's own positionHighlight: a 10px margin, sized in absolute document
		// coordinates (the highlight is position:absolute, so add the scroll offset).

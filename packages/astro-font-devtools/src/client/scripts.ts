...
// (mostly historical scripts in a handful of Noto faces). Lower it to surface rarer writing systems.
const scriptThreshold = 5;

// The script vocabulary, derived from the catalog (base scripts) and ordered by how many families
// carry each (most common first, so `latin` leads). The data is the source of truth; no enum.
...
// Collapse a font's raw subsets to the writing systems it targets: drop auxiliary glyph subsets and
// fold `-ext`/Vietnamese into their base. Turns "has glyphs for" (inclusive, almost everything has
// latin) into "is built for", which is what makes decluttering meaningful.
